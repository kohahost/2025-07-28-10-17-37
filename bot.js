...
async function sendPi() {
    ...
    const apiUrl = `https://api-mainnet.vercel.app/accounts/${senderPublic}`;
    try {
        ...
        if (withdrawAmount <= 0) {
            ...
            if (result && result.transaction_hash !== false) {
                ...
                console.log(`🔗 View Tx: https://api-mainnet.vercel.app/transactions/${result.hash}`);
                console.log(`-------------------------------------------------------------------------------------`)
            } else {
        ...
        setTimeout(sendPi, 1); // Run again after 1 ms
    }
}
